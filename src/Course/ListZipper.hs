...
findLeft f (ListZipper l a r) =
  ...
  in
    if isEmpty ys'
      then IsNotZ
      else let (y :. ys) = ys' in IsZ (ListZipper ys y (reverse xs ++ a :. r))
...
findRight f (ListZipper l a r) =
  ...
  in
    if isEmpty ys'
      then IsNotZ
      else let (y :. ys) = ys' in IsZ (ListZipper (reverse xs ++ a :. l) y ys)
...
moveLeftLoop ::
  ListZipper a
  -> ListZipper a
moveLeftLoop (ListZipper Nil a r) = let (l :. ls) = reverse (a :. r) in ListZipper ls l Nil
...
moveRightLoop ::
  ListZipper a
  -> ListZipper a
moveRightLoop (ListZipper l a Nil) = let (r :. rs) = reverse (a :. l) in ListZipper Nil r rs
...
moveLeftN n l
  | n < 0 = moveRightN (-n) l
  | otherwise = let f = replicate n moveLeft in foldRight (-<<) (IsZ l) f
...
moveRightN n l
  | n < 0 = moveLeftN (-n) l
  | otherwise = let f = replicate n moveRight in foldRight (-<<) (IsZ l) f
...
moveLeftN' n ls@(ListZipper l a r)
  | n < 0 = moveRightN' (-n) ls
  | n > length l = Left (length l)
  | otherwise = Right $ let IsZ ans = moveLeftN n ls in ans
...
moveRightN' ::
  ...
  -> ListZipper a
  -> Either Int (ListZipper a)
moveRightN' n ls@(ListZipper l a r)
  | n < 0 = moveLeftN' (-n) ls
  | n > length r = Left (length r)
  | otherwise = Right $ let IsZ ans = moveRightN n ls in ans
...
-- prop> \lz -> rights (end lz) == Nil
  ...
  ListZipper a
  -> ListZipper a
end lz = ListZipper ls l Nil
  where
    (l :. ls) = reverse (toList lz)
...
instance Extend ListZipper where
  (<<=) ::
    ...
    -> ListZipper a
    -> ListZipper b
  f <<= ls = ListZipper l' a' r'
    where
      l' = unfoldr (f' moveLeft) ls 
      r' = unfoldr (f' moveRight) ls
      a' = f ls 
      f' func lz = let lz' = func lz in
        case lz' of
          IsNotZ -> Empty
          (IsZ lz'') -> Full (f lz'',lz'')
