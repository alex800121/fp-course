...
spChrParser = do
  ...
  case b of
    'u' -> hex
    y   ->
      case toSpecialCharacter b of
        Empty   -> unexpectedCharParser y
...
jsonString ::
  Parser Chars
jsonString =
  between (is '\"') (is '\"') (list (spChrParser ||| noneof (listh "\"\\")))
...
jsonNumber =
  P $
    \st ->
      case readFloats st of
        Empty         -> UnexpectedString st
        Full (n,st')  -> Result st' n
