...
(|||) ::
  ...
  -> Parser a
  -> Parser a
P a ||| P b = P $ \n -> if isErrorResult (a n) then b n else a n
...
list ::
  Parser a
  -> Parser (List a)
list f = (lift2 (:.) f (list f)) ||| pure Nil
...
list1 ::
  Parser a
  -> Parser (List a)
list1 f = lift2 (:.) f (list f ||| pure Nil)
...
satisfy ::
  (Char -> Bool)
  -> Parser Char
satisfy f = character >>= (\n -> if f n then return n else unexpectedCharParser n)
{- OR
satisfy f = do
  c <- character
