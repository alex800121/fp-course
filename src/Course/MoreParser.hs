...
hex ::
  Parser Char
hex = do
  a <- thisMany 4 $ satisfy isHexDigit
  let Full b = readHex a :: Optional Int in
    return (chr b)
...
eof ::
  Parser ()
eof = do
  a <- list (satisfy (const True))
  if isEmpty a then return () else constantParser (ExpectedEof a)
